...
3. **최소 시가총액**: 미국 = UI 필터로 선택(기본 0=미적용, 단위 십억 $) / **한국 3,000억 원**
   - ⚠️ 시총 필터·모든 UI 필터는 **전체 RS 보기**에만 적용. 섹터별 보기는 새로고침 때 구운 sector_snapshot(고정 느슨 필터)만 읽어 무시함
   - ⚠️ NaN 시총은 필터를 통과(제외 안 됨). 미국 시총은 `us_get_meta`에서 `info.marketCap` → `fast_info` → `shares×현재가` 순으로 폴백 수집(레이트리밋으로 None 되는 것 방지). `_refresh_one_meta`는 TTL 안이라도 시총이 비면 재조회 → **US 새로고침 한 번 돌려야 기존 빈 시총이 백필됨**. SPAC/상폐 종목은 원래 시총이 없어 그대로 통과
4. **위험종목·관리종목 제외**
   - 미국: yfinance 메타 기반
