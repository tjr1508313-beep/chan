...
- `scripts/show_sector_rs.py`
  ...
  - `--top-sectors`, `--top-members`, `--min-sector-size`, `--max-tickers`, `--sector`, `--csv-dir` 지원
  - 특정 섹터만 확인할 때 `--sector 반도체` 처럼 실행
- `tests/test_sector_snapshot.py`
  - explicit tickers 경로, 한국 섹터 overlay, `max_tickers`, universe fallback/cache 저장 검증
- `tests/test_show_sector_rs.py`
  - CLI `--sector` 옵션이 요청 섹터 멤버만 출력하는지 검증
...
## LS 업종 API 기반 한국 섹터 보강 ✅ (2026-06-23)

한국 미분류 종목을 줄이기 위해 LS증권 업종 API를 섹터 매핑 생성 경로에 연결.

### 확인
- LS 공식 API 가이드의 업종 시세 경로 `/indtp/market-data` 사용.
- `t8424` 전체업종: `upcode`, `hname` 응답 확인.
- `t1516` 업종별종목시세: `upcode` 입력으로 구성 종목 응답 확인.
- `t1516`은 한 번에 40개 수준이며 `t1516OutBlock.shcode`를 다음 요청에 넣는 방식으로 이어받음.

### 구현
- `screening/ls_sector.py`
  - `.env`의 `LS_APP_KEY` / `LS_APP_SECRET` 로 OAuth 토큰 발급
  - `ls_fetch_industries()`, `ls_fetch_industry_members()`, `ls_build_sector_map()` 추가
  - 넓은 지수/대형주/제조업 버킷은 기본 제외, 필요 시 옵션으로 포함 가능
- `scripts/build_kr_sector_map_ls.py`
  - LS 업종 매핑 후보 생성 및 기존 `data/kr_sector_map.csv`와 병합
  - 기본 병합은 기존 수동/name-rule 행을 보존하고 빠진 종목만 추가
  - `--overwrite`, `--include-krx-theme`, `--include-broad`, `--max-industries` 지원
- `data/kr_sector_map.csv`
  - `scripts/build_kr_sector_map_ls.py --apply` 실행으로 총 2,626개 행 저장
  - source 분포: `ls-industry` 2,434개, `name-rule` 192개

### 검증
- `py scripts\build_kr_sector_map_ls.py --max-industries 1 --preview 5` 성공
- `py scripts\show_sector_rs.py --index-code KS11 --period 20 --top-sectors 8 --top-members 3`
  - 필터 통과 124개 중 미분류 2개 확인

---

## 섹터 RS Streamlit UI 1차 ✅ (2026-06-23)

백엔드/CLI로 확인하던 주도섹터와 섹터 내부 주도주를 Streamlit 화면에 연결.

### 구현
- `screening/ui.py`
  - `ui_load_sector_snapshot()` 캐시 함수 추가
  - 각 미국/한국 RS 랭킹 테이블 위에 `섹터 분석 · 주도섹터와 섹터 내부 주도주` expander 추가
  - `섹터 분석 계산` 체크박스를 켠 경우에만 전체 필터 통과 종목으로 섹터 스냅샷 계산
  - 섹터 요약 테이블: 순위, 섹터, 섹터점수, 양수비율, 종목수, 1등 종목, RS가중
  - 섹터 선택 박스로 해당 섹터 내부 주도주 표 표시
  - 데이터 새로고침 후 섹터 스냅샷 캐시도 함께 clear
- `tests/test_ui_sector_panel.py`
  - 섹터 요약/멤버 화면 표시 포맷 검증

### 사용
- 앱에서 미국주식 또는 한국주식 RS Top 표 바로 위 expander를 연다.
- `섹터 분석 계산`을 체크한다.
- 위 표에서 주도섹터를 보고, 아래 선택 박스에서 섹터 내부 주도주를 확인한다.

---

## 한국 위험종목 필터 LS 재구현 (FDR Dept → LS t1404/t1405) ✅ (2026-06-24)

2026-06-15에 "LS t1404/t1405가 전체 종목 리스트를 반환한다"며 FDR `Dept` 컬럼으로 갈아탔으나,
이는 **`jongchk` 카테고리 파라미터 누락**(시장구분 `gubun`과 혼동)으로 인한 호출 버그였음.
programgarden finance 패키지(최신 v1.6.11) 블록 정의로 정확한 스펙을 확인하고,
`.env` 키로 라이브 호출해 카테고리별 정상 동작을 재확인 → LS로 복귀.

### 확인 (2026-06-24 라이브)
- 경로 `/stock/market-data`, `t1404`(관리/불성실/투자유의), `t1405`(투자경고/매매정지/정리매매)
- `gubun=0`(전체) + `jongchk`로 카테고리 지정 시 **지정 종목만** 반환 (정리매매 5건, 단기과열 12건 등 카테고리별 상이 → 전체 리스트가 아님이 증명)
- 연속조회는 **응답 헤더 `tr_cont`="Y" + `tr_cont_key`** 로 이어받음. InBlock `cts_shcode`만으론 같은 페이지가 반복돼 누락 발생(관리종목 100→실제 106, 매매정지 100→실제 121).
- 라이브 결과: 총 225종목 (is_risk 149 / 배지 76)

### 분류 정책
- **제외(is_risk=True)**: 관리종목(t1404 j=1) · 매매정지(t1405 j=2) · 정리매매(t1405 j=3)
- **참고 배지(제외 X)**: 투자주의환기(t1404 j=4) · 투자경고(t1405 j=1) · 투자주의(t1405 j=4) · 단기과열(t1405 j=7)
- 한 종목이 여러 카테고리에 걸치면 labels 병합, is_risk는 OR

### 변경 파일
- `screening/kr_risk.py` — LS t1404/t1405 기반으로 전면 재작성. `ls_sector`의 OAuth 토큰/base url 재사용, 헤더 연속조회 페이지네이션, 카테고리 단위 실패 격리(graceful degrade)
- `tests/test_kr_risk.py` — FDR mock → LS mock(키 미설정/토큰 실패/분류·병합/카테고리 실패 격리) 4개로 교체
- `docs/ls-openapi-programgarden-reference.md` — t1404/t1405 위험종목 TR 스펙·주의사항 추가
- `CLAUDE.md` 필터 4번 — LS 기준으로 갱신
- 인터페이스(`kr_fetch_risk_flags() -> {code6: {is_risk, labels}}`) 불변 → `batch_kr.screen_refresh_risk_kr` 등 호출부 수정 0줄

### 검증
- 유닛 4개 + 전체 52개 테스트 통과, `py_compile` OK
- 실 API 라이브 통합 검증 완료 (위 수치)

### 배포 잔여 (사용자 몫)
- 로컬은 `.env` 키로 즉시 작동. **Streamlit Cloud**(secrets)와 **GitHub Actions**(refresh-kr) 환경에도
  `LS_APP_KEY`/`LS_APP_SECRET`를 넣어야 그쪽에서도 위험종목이 걸러짐 (미설정 시 필터만 조용히 skip).

### 참고 (programgarden 자료 운영 방침)
- LS TR 참고용 `C:\주식\programgarden` 레포는 **AGPL-3.0** → 스크리닝 레포로 복사·벤더링하지 않고
  상위 폴더에 두고 참고만 한다. 런타임 의존 없음(우리 코드는 clean-room 재구현).
- 최신화: `git -C "C:\주식\programgarden" fetch upstream && git -C "C:\주식\programgarden" merge --ff-only upstream/main

---

## 섹터-우선 스크리닝 화면 (메인 승격) ✅ (2026-06-24, 한국 우선)

기존 곁다리 섹터 expander를 **메인 화면**으로 승격. RS 평면 Top-N 대신 "주도 섹터 안의 주도주"를 본다.
설계/목업 합의: `.claude/plans/temporal-crafting-abelson.md`(승인됨), 확정 비주얼은 v2 목업.

### 동작
- 자산군 섹션 상단 토글: **`섹터별 보기`(기본) ↔ `전체 RS 보기`**(기존 화면 그대로)
- 섹터별 보기 = 히트맵 리본 + 지표카드(상승섹터/평균RS/최강섹터) + 강도순 섹터 아코디언
  - 섹터 정렬 = `sector_score`(상위 5종목 수익률 평균, 랭킹 안정용 5 고정)
  - 카드 펼침 → 그 섹터 RS Top N(전역 컨트롤 기본 20, 5~50) → 종목 클릭 시 기존 5MA·9ATR 차트(픽 로직 재사용)
- 백엔드 무변경: `screen_build_sector_snapshot`/`screen_build_sector_rankings`/`screen_select_sector_members` 재사용
  (멤버는 전부 보관, sector_score는 별도 계산 → 디스플레이 N과 정렬 N 분리)

### 변경 파일
- `screening/ui.py` — `_render_sector_view`, `_render_sector_member_rows`, `_toggle_sector`,
  HTML 빌더(`_build_sector_ribbon_html`/`_build_sector_metrics_html`/`_build_sector_card_html`),
  색조 `_sector_tint`/`_hex_blend`. 기존 `_render_sector_panel`(opt-in expander) 대체. `_render_screening_section`에 뷰 토글.
- `screening/theme.py` — `_SECTOR_CSS`(리본/카드/배지/바) + apply_theme 주입
- `tests/test_ui_sector_panel.py` — tint/blend/지표카드/카드/리본 테스트 +5

### 검증
- 전체 테스트 57개 통과, py_compile OK
- AppTest 헤드리스 스모크: 섹터 뷰 렌더(닫힘) → 토글 펼침(멤버 행) → 멤버 클릭→차트 경로 모두 예외 0
- 시각 확인(브라우저)은 사용자 육안 권장: `streamlit run screening.py`

### 미국/후속
- render는 미국/한국 공통이라 미국도 자동 적용(GICS 11섹터, 거칠어도 동작). 미국 industry 세분화/LS 이관은 별도 스펙.
- 별도 스펙: "US 데이터 yfinance→LS 이관 타당성 검토", 섹터 스냅샷 배치 프리컴퓨트(성능)

### 2차 개편 — 그리드 + 합산 + 느슨 필터 ✅ (2026-06-24, 사용자 피드백 반영)
스탁이지 "오늘의 업종" 참고. 아코디언 → **그리드 타일**로 전환.
- **타일 클릭으로 펼침**(별도 "RS Top N 보기" 버튼 제거, 공간 절약): 타일 = `st.button`(per-key CSS 색조),
  클릭하면 그 줄 아래로 종목 펼침. 한 번에 하나(`_select_sector`). `_build_sector_tiles_css`/`_render_sector_detail`.
- **섹터용 느슨 필터**: 거래대금 300억(코스피 770→124로 컷됨이 "반도체 4종목"의 원인)을 섹터 뷰에선
  기본 30억으로 낮추고 시총 0. 위험/정적제외는 유지. UI에 거래대금 슬라이더.
- **코스피+코스닥 합산**(사용자 결정 "합산+RS정확"): `screen_build_combined_sector_snapshot` 신설.
  `_build_index_ranked`로 시장별 ranked(각 시장 지수 대비 RS 정확) 산출 후 ranked/metadata 합쳐 섹터 재집계.
  → 반도체 4(코스피만)→21(코스피4+코스닥17), sector_score +26% rank5 주도섹터로 정상화.
  - 진단: kr_sector_map 분류는 정확. "4종목"은 코스피에 큰 반도체가 4개뿐(나머지는 코스닥)이었던 것.
- 검증: 전체 57개 테스트 통과(합산 머지/타일CSS 신규 포함), AppTest 합산 그리드 스모크(46섹터·반도체21·클릭→펼침→차트) 예외 0.

### 3차 — precompute & store (화면은 읽기만) ✅ (2026-06-24)
사용자 요청: "사이트는 보여주기만, 무거운 계산은 새로고침 때 다 저장". 기존 `index_chart_snapshot`/
`screening_metrics` precompute 패턴과 동일하게 섹터 결과 자체를 저장.
- **새 테이블 `sector_snapshot`**(scope·period·summary_json·members_json·updated_at):
  `cache_save_sector_snapshot`/`cache_load_sector_snapshot`(cache.py). 로드 시 `dtype=False`로 티커 앞자리 0 보존.
- **굽기**: `sector.screen_rebuild_sector_snapshot(market)` — KR 합산 1개 + US 2개 scope 계산·저장.
  새로고침 경로 양쪽(`refresh_cache.py` `_refresh_us`/`_refresh_kr`, 로컬 `_refresh_worker`)에 추가 → **데이터 새로고침 = 섹터까지 저장 완료**.
- **화면**: `_render_sector_view`가 `ui_load_stored_sector_snapshot`로 읽기만(실측 로드 0.015s, 굽기 1.3s).
  계산 기준 완전 고정(RS 20일·거래대금 30억). 라이브 슬라이더 제거. 미저장 시 "지금 계산" 폴백.
- **표시**: 수익률 상위 12개 섹터(`전체 섹터 보기` 토글로 전체). 클릭 → 저장된 그 섹터 종목 즉시 펼침.
- 검증: 58개 테스트 통과(캐시 라운드트립+티커0보존 신규), AppTest 저장-읽기 스모크(상위12·반도체21·클릭→차트) 예외 0.

### 4차 — 멤버 필터/지표 폰트/지수차트 사라짐 ✅ (2026-06-24, 사용자 피드백)
- **멤버 필터 = 코스피 절대수익률 기준**: 기존 rs(각 시장 지수 대비)는 KQ11 지수가 -23%(2026-06-23 마지막 봉 이상치)로 망가져
  코스닥 종목 rs가 +23%p 부풀려져 필터 우회(티씨케이 -16%인데 rs +6.8%p로 통과). → **종목 절대 20일 수익률 ≥ KOSPI(KS11) 20일 -5%p**
  로 변경(`_index_period_return`/`_render_sector_detail`). 비금속: 원익QnC/티씨케이 제외, 3종목만.
- **섹터당 상위 10개**만 펼침(이전 전체).
- **타일/지표 폰트 확대·굵게·Pretendard**: 타일 섹터명 20px/800, 지표카드 값 28px/800. `전체 섹터 보기` 토글을 보기방식 라디오 옆으로 이동(세로 절약).
- **지수 미니차트 사라짐 해결**: lightweight-charts iframe이 섹터 타일 클릭(리런)으로 사라지던 문제 →
  **정적 SVG 캔들**(`_index_chart_svg`)로 교체. 항상 떠 있음(인터랙션은 빠짐). 과거 정적 SVG↔대화형 왕복 이력의 재정착.
- ⚠️ 데이터 이상치: 2026-06-23 지수 마지막 봉이 KS11 -10%/KQ11 -8%로 찍힘(깨진 봉 의심) → 새로고침 권장.
- 검증: 58개 테스트 통과, AppTest(지수SVG차트 + 타일클릭 후 차트 유지 True + 비금속 3종목) 예외 0.`
