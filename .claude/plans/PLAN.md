...
### 잠재 옵션 — 한국투자증권 (KIS) OpenAPI 도입 (2026-05-11 메모)
**현 상태**: 보류. 사용자 한투 계좌 있음. 도입 가치 정리만 해둠.
- **속도**: FDR 병렬 8워커와 비슷 (KIS REST 1초 20건). 큰 향상 없음.
- **진짜 이점**:
  - 공식 거래소 직결 데이터 (FDR 네이버 우회 의존성 탈피)
  - 분봉/실시간 시세 — 일봉 외 신호 활용 가능
  - 관리종목/위험종목 상태 깔끔하게 제공 → Phase 2.10 보류 항목 해결
  - 매매 연동 (자동매매 확장 시)
  - 외국인/기관 수급, 호가, 체결정보
- **도입 비용**: 한투 계좌 → KIS Developers 앱 등록 → APP_KEY/SECRET 발급 → OAuth 토큰 24h 자동 갱신 → `screening/data_kr_kis.py` 신설
- **검토 시점 트리거**:
  - FDR 가 작동 안 하기 시작할 때
  - 분봉/실시간 신호가 필요해질 때
  - 관리종목 필터를 제대로 켜고 싶을 때
  - 스크리닝에서 본 종목을 바로 매수하고 싶을 때
