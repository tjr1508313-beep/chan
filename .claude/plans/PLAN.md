...
기존 단순 비율 RS와 Minervini 가중 RS를 나란히 표시. 헤더 클릭으로 정렬 전환.
...
- `ui.py`:
  - 헤더 행을 별도 container(`scr_rank_header_*`)로 분리 — 데이터 셀 CSS 충돌 방지
  - RS / RS가중 헤더는 sort-toggle `st.button`, 활성 컬럼 ▼ 표시
  - 정렬 기준 `scr_{code}_sort_col` 세션 스테이트, 재정렬 후 순위 재번호
  - "RS가중" 셀 추가, 252일 부족 종목은 "—" 표시
- `theme.py`: `scr_rank_header_*` 컨테이너 정렬 버튼 CSS 추가

### 동작 방식
- 기본 정렬: `rs` (단순 비율 20일)
- RS 또는 RS가중 헤더 클릭 → 해당 컬럼 내림차순, 순위 재번호
- 252영업일 미만 종목은 `rs_weighted = NaN` → 정렬 시 맨 아래
...
- `screening/cache.py` — `metadata.caution_flags` 컬럼 + `_migrate_caution_flags_column` + `update_risk_flags`
